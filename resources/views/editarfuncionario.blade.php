@extends('layouts.app', ['activePage' => 'user', 'title' => 'Sistema de Sumate', 'navName' => 'Formulario', 'activeButton' => 'laravel'])
@section('content')
        ...
        <div class="container-fluid">
            <form action="{{ route('update_usuario') }}" method="post">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <input type="hidden" name="user_id" id="user_id" value="{{$solicitud->id}}" >
                        <div class="row">
                            <div class="col-md-6">
                                <label for=""><span class="text-danger">(*)</span> Nombre Completo :</label>
                                <input type="text" name="name" id="name" class="form-control" style="text-transform: uppercase;" value="{{$solicitud->name}}" >
                                @error('name')
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="email" id="email" class="form-control" value="{{$solicitud->email}}" >
                                @error('email')
                                    <p class='text-danger inputerror'>{{ $message }} </p>
                                @enderror
                            </div>
                        ...
                        <div class="text-center">
                            <button type="submit" class="btn btn-lila-outline btn-oval">Actualizar Registro</button>
                        </div>
                    </div>    
                </div>
            </form>
